...
''' From Ben Basanta and Derrick Hicks, copied to here for stability '''
# Benjamin Basanta 07/2017 - Create mover form XML in Pyrosetta functions.
# Derrick Hicks 08/2017 <- This flag parsing code was adapted outside of the function
...
def main():
   ...
   if args.state == 'OFF':
      ...
      max_dist = 0
   # else:
   #    script_vars.append("cst={0}".format(args.cst))
   #    cst_coords = {}
   #    with open(args.cst, 'r') as constraint_file:
   #       constraint_lines = constraint_file.readlines()

   #    for Line in constraint_lines:
   #       if len(Line.strip()):
   #          if Line.startswith('CoordinateConstraint'):
   #             CstType, Name1, Res1, Name2, Res2, X, Y, Z = tuple( Line.split()[:8] )
   #             Res1, Chain1, Res2, Chain2 = split_chain_and_residue([Res1,Res2])
   #             # print (Res1, Chain1)
   #             ResNum = Res1.rjust(4, ' ')
   #             key = Chain1+ResNum+'__'+X+'_'+Y+'_'+Z
   #             cst_coords[key] = np.array((float(X),float(Y),float(Z)))
   #          else:
   #             assert Line.startswith('CoordinateConstraint'), Line
   ...
   for i in range(1, args.nstruct+1):
      ...
      # pdb_atom_coords = {}
      # for resi in range(1, protein_chain.size()+1):
      #    atom_key = 'B' + str(resi).rjust(4, ' ')
      #    pdb_atom_coords[atom_key] = np.array( list( protein_chain.residue(resi).xyz('CA') ) )

      # if args.state == 'ON':
      #    print (pdb_atom_coords)
      #    sys.exit()

      #    distances = [ np.linalg.norm(pdb_atom_coords[meta_key.split('__')[0]] - cst_coords[meta_key]) for meta_key in cst_coords ]
         
      #    min_dist = round(min(distances), 3)
      #    med_dist = round(np.median(distances), 3)
      #    max_dist = round(max(distances), 3)

      #    z_coords = [pdb_atom_coords[meta_key.split('__')[0]][-1] for meta_key in cst_coords ]

      #    highest_constrained_atom = max(z_coords)
      #    lowest_constrained_atom = min(z_coords)
      #    tilt_depth = highest_constrained_atom - lowest_constrained_atom
