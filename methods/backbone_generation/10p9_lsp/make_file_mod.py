...
<FILTERS>
    ...
    <RepeatParameter name="radius" param_type="radius" min="100" numb_repeats="4" confidence="1"/> #replace with correct number of repeats
    <RepeatParameter name="rise" param_type="rise" numb_repeats="4" confidence="0"/>
    <RepeatParameter name="omega" param_type="omega" max="0.1" numb_repeats="4" confidence="0"/>
</FILTERS>
<MOVERS>
...
def copy_necessary_files(name):
    ...
    os.system("cp rd1_files/abinitio_remodel_cen_stage0.wts "+ name +"/abinitio_remodel_cen_stage0.wts")
...
def add_flags(path_name,name):
    ...
    for ii in range(1,design_length*2-1):
        fl.write("{},".format(ii))
                ...
                if h_diff <= 3:
                    for l2 in l2s:
                        for idir in range(num_dirs):
                            name = "X_{}_{}_{}_{}_c{}".format(h1,l1,h2,l2,d)
